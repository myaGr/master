...
class Parse100CData(object):
    ...
    def save_320_to_df_old(self):
        ...
        startflag = 0
        # 320内应含有的字段
        keys_name = ['GPSTime', 'Latitude', 'Longitude', 'H-Ell', 'Roll', 'Pitch', 'Heading'
                    , 'Vel-N', 'Vel-E', 'Vel-D']
        required_keys = ["time", "lat", "lon", "height", "yaw", "pitch", "roll", "Q", "NorthVelocity", "EastVelocity",
                         "GroundVelocity", "H-Ell", "AccX", "GyroX", "AccY", "GyroY", "AccZ", "GyroZ"]
        keys = ["time", "lat", "lon", "H-Ell", "roll", "pitch", "yaw", "NorthVelocity", "EastVelocity",
                "GroundVelocity"]
        values = {}
        for line in lines:
            li = line.replace('\n', '').split(" ")
            ...
            if startflag == 1 and len(li) >= 10 and li[0] != '(sec)':
                for k in range(len(keys)):
                    values[keys[k]].append(float(li[k]))

            if "GPSTime" in li and len(li) >= 10:  # 寻找320数据字头，共10列
                diff_keys = set(keys_name) - set(li)
                if len(diff_keys) > 0:  # 判断是否缺少字段， 如果缺少直接返回
                    print('missing values:', diff_keys)
                    return diff_keys
        ...
        file_length = len(values[keys[0]])
        values['Q'] = [1] * file_length
