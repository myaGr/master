...
static string basename(const string& s) {
  ...
  string b = s;
  size_t i = b.find_last_not_of(slash);
  if (i == string::npos) {
    if (b[0] == slash)
      b.erase(1);
    return b;
...
void OptionParser::print_args() const {
  cout << prog();
  for (list<string>::const_iterator it = _args.begin(); it != _args.end(); ++it) {
    cout << " " << *it;
  }
  cout << endl;
}
