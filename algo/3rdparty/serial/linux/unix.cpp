...
{
  if (port_.empty ()) {
    LOG_F(ERROR, "Empty port is invalid.");
  }
  if (is_open_ == true) {
    LOG_F(ERROR, "Serial port already open.");
  }
  ...
  if (fd_ == -1) {
    ...
    case ENFILE:
    case EMFILE:
      LOG_F(ERROR, "Too many file handles open.");
      return;
    default:
      LOG_F(ERROR, "Open serial port fail errno %d %s.", errno, strerror(errno));
      return;
    }
  }
...
Serial::SerialImpl::read (uint8_t *buf, size_t size)
{
  ...
  total_timeout_ms += timeout_.read_timeout_multiplier * total_timeout_ms;
