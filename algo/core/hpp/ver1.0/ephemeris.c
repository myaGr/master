...
#define RELATIVE_TOL_KEPLER 1E-13         /* relative tolerance for Kepler equation */
...
static double var_uraeph(int sys, int ura)
{
	...
	if (sys==SYS_GAL) { /* galileo sisa (ref [7] 5.1.11) */
		...
		if (ura<= 74) return SQR(0.5+(ura- 50.0)*0.02);
		if (ura<= 99) return SQR(1.0+(ura- 75.0)*0.04);
		if (ura<=125) return SQR(2.0+(ura-100.0)*0.16);
...
extern void alm2pos(gtime_t time, const alm_t *alm, double *rs, double *dts)
{
    ...
    gnss_util_trace(4,"alm2pos : time=%s sat=%2d\n",time_str(time,3),alm->sat);
    ...
    for (n=0,E=M,Ek=0.0;fabs(E-Ek)>RELATIVE_TOL_KEPLER&&n<MAX_ITER_KEPLER;n++) {
        Ek=E; E-=(E-alm->e*sin(E)-M)/(1.0-alm->e*cos(E));
    }
    if (n>=MAX_ITER_KEPLER) {
        gnss_util_trace(2,"alm2pos: kepler iteration overflow sat=%2d\n",alm->sat);
        return;
    }
...
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    ...
    gnss_util_trace(5,"eph2clk : time=%s sat=%2d\n",time_str(time,3),eph->sat);
                    ...
                    double *var)
{
    ...
    gnss_util_trace(5,"eph2pos : time=%s sat=%2d\n",time_str(time,3),eph->sat);
    ...
    switch ((sys=satsys(eph->sat,&prn))) {
        ...
        case SYS_CMP: mu=MU_BDS; omge=OMGE_BDS; break;
    ...
    if (n>=MAX_ITER_KEPLER) {
        gnss_util_trace(2,"eph2pos: kepler iteration overflow sat=%2d\n",eph->sat);
        return;
    }
    sinE=sin(E); cosE=cos(E);
    
    gnss_util_trace(5,"kepler: sat=%2d e=%8.5f n=%2d del=%10.3e\n",eph->sat,eph->e,n,E-Ek);
...
extern double geph2clk(gtime_t time, const geph_t *geph)
{
    ...
    gnss_util_trace(5,"geph2clk: time=%s sat=%2d\n",time_str(time,3),geph->sat);
                     ...
                     double *var)
{
    ...
    gnss_util_trace(4,"geph2pos: time=%s sat=%2d\n",time_str(time,3),geph->sat);
...
extern double seph2clk(gtime_t time, const seph_t *seph)
{
    ...
    gnss_util_trace(5,"seph2clk: time=%s sat=%2d\n",time_str(time,3),seph->sat);
                     ...
                     double *var)
{
    ...
    gnss_util_trace(4,"seph2pos: time=%s sat=%2d\n",time_str(time,3),seph->sat);
...
static eph_t* seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    gnss_util_trace(5,"seleph  : time=%s sat=%2d iode=%d\n",time_str(time,3),sat,iode);
    ...
    if (iode>=0||j<0) {
        gnss_util_trace(3,"no broadcast ephemeris: %s sat=%2d iode=%3d\n",time_str(time,0),
              sat,iode);
        return NULL;
    }
    return nav->eph+j;
...
static geph_t *selgeph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    gnss_util_trace(4,"selgeph : time=%s sat=%2d iode=%2d\n",time_str(time,3),sat,iode);
    ...
    if (iode>=0||j<0) {
        gnss_util_trace(3,"no glonass ephemeris  : %s sat=%2d iode=%2d\n",time_str(time,0),
              sat,iode);
        return NULL;
    }
    return nav->geph+j;
...
static seph_t *selseph(gtime_t time, int sat, const nav_t *nav)
{
    ...
    gnss_util_trace(4,"selseph : time=%s sat=%2d\n",time_str(time,3),sat);
    ...
    if (j<0) {
        gnss_util_trace(3,"no sbas ephemeris     : %s sat=%2d\n",time_str(time,0),sat);
        return NULL;
    }
    return nav->seph+j;
#else
...
/* select most suitable eph from nav info for specified SV, added by xukun */
extern void* selephfromnav(gtime_t teph, int sat, const nav_t *nav, int iode)
{
	eph_t  *eph;
	geph_t *geph;
	seph_t *seph;
                  ...
                  double *dts)
{
    ...
    gnss_util_trace(5,"ephclk  : time=%s sat=%2d\n",time_str(time,3),sat);
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    gnss_util_trace(5,"ephpos  : time=%s sat=%2d iode=%d\n",time_str(time,3),sat,iode);
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
        ...
        if (iode==-2) gnss_rtk_set_refeph_iode(sat,sys==SYS_CMP?((((int32_t)eph->toes)/720) % 240):eph->iode);
        ...
        *svh=eph->svh;
        ...
        if (iode==-2) gnss_rtk_set_refeph_iode(sat,geph->iode);
                        ...
                        double *rs, double *dts, double *var, int *svh)
{
    ...
    gnss_util_trace(4,"satpos_sbas: time=%s sat=%2d\n",time_str(time,3),sat);
    ...
    if (i>=nav->sbssat.nsat) {
        gnss_util_trace(2,"no sbas correction for orbit: %s sat=%2d\n",time_str(time,0),sat);
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    gnss_util_trace(4,"satpos_ssr: time=%s sat=%2d\n",time_str(time,3),sat);
    
    ssr=nav->ssr+sat-1;
    
    if (!ssr->t0[0].time) {
        gnss_util_trace(2,"no ssr orbit correction: %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    if (!ssr->t0[1].time) {
        gnss_util_trace(2,"no ssr clock correction: %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    /* inconsistency between orbit and clock correction */
    if (ssr->iod[0]!=ssr->iod[1]) {
        gnss_util_trace(2,"inconsist ssr correction: %s sat=%2d iod=%d %d\n",
    ...
    if (fabs(t1)>MAXAGESSR||fabs(t2)>MAXAGESSR) {
        gnss_util_trace(2,"age of ssr error: %s sat=%2d t=%.0f %.0f\n",time_str(time,0),
              sat,t1,t2);
        *svh=-1;
        return 0;
    }
    ...
    if (asg_norm(deph,3)>MAXECORSSR||fabs(dclk)>MAXCCORSSR) {
        gnss_util_trace(3,"invalid ssr correction: %s deph=%.1f dclk=%.1f\n",
              time_str(time,0),asg_norm(deph,3),dclk);
        *svh=-1;
        return 0;
    }
    /* satellite postion and clock by broadcast ephemeris */
    ...
    *var=var_urassr(ssr->ura);
    ...
    gnss_util_trace(5,"satpos_ssr: %s sat=%2d deph=%6.3f %6.3f %6.3f er=%6.3f %6.3f %6.3f dclk=%6.3f var=%6.3f\n",
...
extern int satpos(gtime_t time, gtime_t teph, int sat, int ephopt,
                  ...
                  int *svh)
{
    gnss_util_trace(5,"satpos  : time=%s sat=%2d ephopt=%d\n",time_str(time,3),sat,ephopt);
    
    *svh=0;
    
    switch (ephopt) {
        case EPHOPT_BRDC_IODE:  return ephpos     (time,teph,sat,nav,-2,rs,dts,var,svh);
...
*-----------------------------------------------------------------------------*/
#if 0
extern void ag_gnss_satposs(gtime_t teph, const obsd_t *obs, int n, const nav_t *nav,
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    gtime_t time[2*MAXOBS]={{0}};
    double dt,pr;
    int i,j,newopt=ephopt;
    
    gnss_util_trace(3,"ag_gnss_satposs : teph=%s n=%d ephopt=%d\n",time_str(teph,3),n,ephopt);
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if (j>=NFREQ) {
            gnss_util_trace(2,"no pseudorange %s sat=%2d\n",time_str(obs[i].time,3),obs[i].sat);
            continue;
        }
        ...
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
            gnss_util_trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
        time[i]=timeadd(time[i],-dt);
        
		newopt = EPHOPT_BRDC;
		if (ephopt==EPHOPT_BRDC_IODE && obs[i].rcv==2)
		{
			newopt = EPHOPT_BRDC_IODE;
		}

        /* satellite position and clock at transmission time */
        if (!satpos(time[i],teph,obs[i].sat,newopt,nav,rs+i*6,dts+i*2,var+i,
                    svh+i)) {
            gnss_util_trace(3,"no ephemeris %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        gnss_util_trace(4,"%s sat=%2d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.3f svh=%02X\n",
...
extern void ag_gnss_satposs(gtime_t teph, const obsd_t *obs, int n, const nav_t *nav,
	int ephopt, double *rs, double *dts, double *var, int *svh, double *azel)
{
	gtime_t        time;
	double         pr;
	uint8_t             gnssMode;
	uint32_t            leapsecs = 0;
	double            dtClk, dopp_corr, tot = 0.0;
	ECEF           svp;
	int            i, j;
	sat_data_t*    sp = NULL;
	...
	for (i = 0; i < n && i < 2 * MAXOBS; i++) {
		...
		svh[i] = 0; dtClk = 0.0; dopp_corr = 0.0;
		...
		gnssMode = GNSS_SYS2MODE(obs[i].sys);

		if (gnssMode == GLN_MODE && pTime != NULL)
		{
			leapsecs = gnss_tm_leap_secs(GPS_MODE, &(pTime->utcTime));
		}
		...
		case GLN_MODE: {pTime = gnss_tm_get_time();
			tot = (time.time - SECS_IN_DAY * 3 + SECS_IN_HOUR * 3 -
				leapsecs) % SECS_IN_DAY + time.sec; break; }
		default: continue;
		}

		if ((sp = gnss_sd_get_sv_data(gnssMode, obs[i].prn, j)) == NULL) continue;

		azel[0 + i * 2] = sp->d_cos.az; azel[1 + i * 2] = sp->d_cos.elev;

		if (obs[i].rcv == 1) {
			rs[0 + i * 6] = sp->ecef.p[0]; rs[1 + i * 6] = sp->ecef.p[1]; rs[2 + i * 6] = sp->ecef.p[2];
			rs[3 + i * 6] = 0.0; rs[4 + i * 6] = 0.0; rs[5 + i * 6] = 0.0;
			if (j == 0)
			{
				dts[0 + i * 2] = sp->svt[0]; dts[1 + i * 2] = sp->svt[1];
			}
			else
			{
				dtClk = gnss_Sd_Clk(gnssMode, obs[i].prn, tot, &dopp_corr, 0);
				...
				dts[0 + i * 2] = dtClk; dts[1 + i * 2] = dopp_corr;
			}
		}
		else if (obs[i].rcv == 2) {
			if(gnssMode == GLN_MODE)
			{
				dtClk = gnss_Sd_Clk(gnssMode, obs[i].prn, tot, &dopp_corr, j);
				tot += dtClk;
				dtClk = -gnss_Sd_Pos_e(gnssMode, obs[i].prn, tot, &svp, &dopp_corr, j);
			}
			else
			{
				dtClk = gnss_Sd_Clk(gnssMode, obs[i].prn, tot, &dopp_corr, j);
			...
			rs[0 + i * 6] = svp.p[0]; rs[1 + i * 6] = svp.p[1]; rs[2 + i * 6] = svp.p[2];
			rs[3 + i * 6] = 0.0; rs[4 + i * 6] = 0.0; rs[5 + i * 6] = 0.0;
			dts[0 + i * 2] = dtClk; dts[1 + i * 2] = dopp_corr;

			//printf("satposs check: sat %03d, tor %.3f, P %.10f %.10f %.10f, dt %.10f %.10f, tot %.16f toe %.16f\n", obs[i].sat, tot, rs[0 + i * 6], rs[1 + i * 6], rs[2 + i * 6], dtClk, dopp_corr, tot, sp->toe);
		}

		if (EPHOPT_BRDC_TOE == ephopt && obs[i].rcv == 2)
		{
			gnss_rtk_set_refeph_toe(obs[i].sat, sp->toe);
		}
	}
}
