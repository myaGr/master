...
static int LD(int n, const double *Q, double *L, double *D)
{
    ...
    Sys_Free(A);
    if (info) {
    	SYS_LOGGING(OBJ_RTK,LOG_ERROR,"LD factorization error");
    }
    return info;
}
                  ...
                  const double *zs, double *zn, double *s)
{
	...
	if (NULL == S || NULL == dist || NULL == zb || NULL == z || NULL == step) {
		if (S)   Sys_Free(S);
		if (dist) Sys_Free(dist);
		if (zb)  Sys_Free(zb);
		if (z)   Sys_Free(z);
		if (step) Sys_Free(step);
		return -1;
	}
    ...
    Sys_Free(S); Sys_Free(dist); Sys_Free(zb); Sys_Free(z); Sys_Free(step);
    
    if (c>=LOOPMAX) {
    	SYS_LOGGING(OBJ_RTK,LOG_INFO,"search loop count=%d overflow", c);
        return -1;
    }
    return 0;
}
                  ...
                  double *s)
{
	...
	if (NULL == L || NULL  == D || NULL == Z || NULL == z || NULL == E) {
		if (L) Sys_Free(L);
		if (D) Sys_Free(D);
		if (Z) Sys_Free(Z);
		if (z) Sys_Free(z);
    ...
    if (!(info=LD(n,Q,L,D))) {
		...
		double adop = 1.0;
		for (i = 0; i < n; i++) adop *= D[i];
		if (adop > 0.0) ADOP = 100.0*pow(adop, 1.0 / (2.0 * n));
...
#else
        Fast_Rtk_matmul("TN", n, 1, n, 1.0, Z, a, 0.0, z); /* z=Z'*a */
#endif
    ...
    Sys_Free(L); Sys_Free(D); Sys_Free(Z); Sys_Free(z); Sys_Free(E);
    return info;
}
/* lambda reduction ------------------------------------------------------------
