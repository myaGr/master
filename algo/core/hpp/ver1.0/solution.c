...
#define SQR_OLD(x)     ((x)<0.0?-(x)*(x):(x)*(x))
...
static int decode_nmearmc(char **val, int n, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_nmearmc: n=%d\n",n);
    ...
    if ((act!='A'&&act!='V')||(ns!='N'&&ns!='S')||(ew!='E'&&ew!='W')) {
        gnss_util_trace(2,"invalid nmea gprmc format\n");
        return 0;
    }
    ...
    sol->time=utc2gpst(gnss_epoch2time(ep));
    pos2ecef(pos,sol->rr);
    ...
    gnss_util_trace(5,"decode_nmearmc: %s rr=%.3f %.3f %.3f stat=%d ns=%d vel=%.2f dir=%.0f ang=%.0f mew=%c mode=%c\n",
...
static int decode_nmeazda(char **val, int n, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_nmeazda: n=%d\n",n);
    ...
    gnss_util_trace(5,"decode_nmeazda: %s\n",time_str(sol->time,0));
...
static int decode_nmeagga(char **val, int n, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_nmeagga: n=%d\n",n);
    ...
    if ((ns!='N'&&ns!='S')||(ew!='E'&&ew!='W')) {
        gnss_util_trace(2,"invalid nmea gpgga format\n");
        return 0;
    }
    if (sol->time.time==0.0) {
        gnss_util_trace(2,"no date info for nmea gpgga\n");
        return 0;
    }
    ...
    gnss_time2epoch(sol->time,ep);
    septime(tod,ep+3,ep+4,ep+5);
    time=utc2gpst(gnss_epoch2time(ep));
    ...
    gnss_util_trace(5,"decode_nmeagga: %s rr=%.3f %.3f %.3f stat=%d ns=%d hdop=%.1f ua=%c um=%c\n",
          time_str(sol->time,0),sol->rr[0],sol->rr[1],sol->rr[2],sol->stat,sol->ns,
          hdop,ua,um);
...
static int decode_nmea(char *buff, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_nmea: buff=%s\n",buff);
	memset(val, 0, sizeof(char*)*MAXFIELD);
...
static char *decode_soltime(char *buff, const solopt_t *opt, gtime_t *time)
{
    ...
    gnss_util_trace(4,"decode_soltime:\n");
    ...
    if (opt->posf==SOLF_GSIF) {
        ...
        *time=timeadd(gnss_epoch2time(v),-12.0*3600.0);
        if (!(p=strchr(buff,':'))||!(p=strchr(p+1,':'))) return NULL;
        for (p++;isdigit((int)*p)||*p=='.';) p++;
        return p+len;
    }
    ...
    if (n>=2&&0.0<=v[0]&&v[0]<=3000.0&&0.0<=v[1]&&v[1]<604800.0) {
        *time= gnss_gpst2time((int)v[0],v[1]);
        return p;
    }
    return NULL;
}
...
static int decode_solxyz(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_solxyz:\n");
    ...
    if (i+3<n) {
        ...
        if (i+3<n) {
            P[1]=P[3]= SQR_OLD(val[i]); i++; /* sdxy */
            P[5]=P[7]= SQR_OLD(val[i]); i++; /* sdyz */
            P[2]=P[6]= SQR_OLD(val[i]); i++; /* sdzx */
        }
        covtosol(P,sol);
    }
...
static int decode_solllh(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_solllh:\n");
    ...
    if (i+3<n) {
        ...
        if (i+3<n) {
            Q[1]=Q[3]= SQR_OLD(val[i]); i++; /* sdne */
            Q[2]=Q[6]= SQR_OLD(val[i]); i++; /* sdeu */
            Q[5]=Q[7]= SQR_OLD(val[i]); i++; /* sdun */
        }
        covecef(pos,Q,P);
        covtosol(P,sol);
    }
...
static int decode_solenu(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_solenu:\n");
    ...
    if (i+3<n) {
        ...
        Q[8]=val[i]*val[i]; i++; /* sdu */
        if (i+3<n) {
            Q[1]=Q[3]= SQR_OLD(val[i]); i++; /* sden */
            Q[5]=Q[7]= SQR_OLD(val[i]); i++; /* sdnu */
            Q[2]=Q[6]= SQR_OLD(val[i]); i++; /* sdue */
        }
        covtosol(Q,sol);
    }
    if (i<n) sol->age  =(float)val[i++];
...
static int decode_solsss(char *buff, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_solssss:\n");
    
    if (sscanf(buff,"$POS,%d,%lf,%d,%lf,%lf,%lf,%lf,%lf,%lf",&week,&tow,&solq,
               pos,pos+1,pos+2,std,std+1,std+2)<6) {
        return 0;
    }
    if (week<=0||asg_norm(pos,3)<=0.0||solq==SOLQ_NONE) {
        return 0;
    }
    sol->time= gnss_gpst2time(week,tow);
    for (i=0;i<6;i++) {
        sol->rr[i]=i<3?pos[i]:0.0;
        sol->qr[i]=i<3?(float)SQR_OLD(std[i]):0.0f;
        sol->dtr[i]=0.0;
    }
    sol->ns=0;
...
static int decode_solgsi(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_solgsi:\n");
...
static int decode_solpos(char *buff, const solopt_t *opt, sol_t *sol)
{
    ...
    gnss_util_trace(4,"decode_solpos: buff=%s\n",buff);
...
static void decode_refpos(char *buff, const solopt_t *opt, double *rb)
{
    ...
    gnss_util_trace(3,"decode_refpos: buff=%s\n",buff);
...
static int decode_sol(char *buff, const solopt_t *opt, sol_t *sol, double *rb)
{
    ...
    gnss_util_trace(4,"decode_sol: buff=%s\n",buff);
...
static void decode_solopt(char *buff, solopt_t *opt)
{
    ...
    gnss_util_trace(4,"decode_solhead: buff=%s\n",buff);
                    ...
                    int qflag, const solopt_t *opt, solbuf_t *solbuf)
{
    ...
    gnss_util_trace(4,"inputsol: data=0x%02x\n",data);
    ...
    if (!strcmp((char *)solbuf->buff,MSG_DISCONN)) {
        gnss_util_trace(3,"disconnect received\n");
        return -1;
    }
    /* decode solution */
    sol.time=solbuf->time;
...
static int sort_solbuf(solbuf_t *solbuf)
{
    ...
    gnss_util_trace(4,"sort_solbuf: n=%d\n",solbuf->n);
    
    if (solbuf->n<=0) return 0;
    Sys_Free(solbuf->data);
    if (!(solbuf_data=(sol_t *)Sys_Malloc(sizeof(sol_t)*solbuf->n))) {
        gnss_util_trace(1,"sort_solbuf: memory allocation error\n");
                    ...
                    double tint, int qflag, solbuf_t *solbuf)
{
    ...
    gnss_util_trace(3,"readsolt: nfile=%d\n",nfile);
    
    initsolbuf(solbuf,0,0);
    
    for (i=0;i<nfile;i++) {
        if (!(fp=fopen(files[i],"rb"))) {
            gnss_util_trace(1,"readsolt: file open error %s\n",files[i]);
            continue;
        }
        ...
        if (!readsoldata(fp,ts,te,tint,qflag,&opt,solbuf)) {
            gnss_util_trace(1,"readsolt: no solution in %s\n",files[i]);
        }
        fclose(fp);
    }
...
extern int readsol(char *files[], int nfile, solbuf_t *sol)
{
    ...
    gnss_util_trace(3,"readsol: nfile=%d\n",nfile);
...
extern int addsol(solbuf_t *solbuf, const sol_t *sol)
{
    ...
    if (solbuf->n>=solbuf->nmax) {
        ...
        Sys_Free(solbuf->data);
        if (!(solbuf_data=(sol_t *)Sys_Malloc(sizeof(sol_t)*solbuf->nmax))) {
            gnss_util_trace(1,"addsol: memory allocation error\n");
            solbuf->n=solbuf->nmax=0;
            return 0;
        }
        solbuf->data=solbuf_data;
...
{
    gnss_util_trace(4,"getsol: index=%d\n",index);
...
extern void initsolbuf(solbuf_t *solbuf, int cyclic, int nmax)
{
    ...
    gnss_util_trace(3,"initsolbuf: cyclic=%d nmax=%d\n",cyclic,nmax);
    ...
    if (cyclic) {
        if (nmax<=2) nmax=2;
        if (!(solbuf->data=(sol_t *)Sys_Malloc(sizeof(sol_t)*nmax))) {
            gnss_util_trace(1,"initsolbuf: memory allocation error\n");
            return;
        }
        solbuf->nmax=nmax;
    }
...
extern void freesolbuf(solbuf_t *solbuf)
{
    int i;
    if (!solbuf) return;
    gnss_util_trace(3,"freesolbuf: n=%d\n",solbuf->n);
    Sys_Free(solbuf->data);
    solbuf->n=solbuf->nmax=solbuf->start=solbuf->end=solbuf->nb=0;
    solbuf->data=NULL;
    for (i=0;i<3;i++) {
...
{
    if (!solstatbuf) return;
    gnss_util_trace(3,"freesolstatbuf: n=%d\n",solstatbuf->n);
    solstatbuf->n=solstatbuf->nmax=0;
    Sys_Free(solstatbuf->data);
}
...
static int sort_solstat(solstatbuf_t *statbuf)
{
    ...
    gnss_util_trace(4,"sort_solstat: n=%d\n",statbuf->n);
    
    if (statbuf->n<=0) return 0;
    Sys_Free(statbuf->data);
    if (!(statbuf_data=(solstat_t *)Sys_Malloc(sizeof(solstat_t)*statbuf->n))) {
        gnss_util_trace(1,"sort_solstat: memory allocation error\n");
...
static int decode_solstat(char *buff, solstat_t *stat)
{
  if (NULL != buff) {
    ...
    gnss_util_trace(4, "decode_solstat: buff=%s\n", buff);
    ...
    if (n < 15) {
      gnss_util_trace(2, "invalid format of solution status: %s\n", buff);
      return 0;
    }
    if ((sat = satid2no(id)) <= 0) {
      gnss_util_trace(2, "invalid satellite in solution status: %s\n", id);
      return 0;
    }
    *stat = stat0;
    stat->time = gnss_gpst2time(week, tow);
...
static void addsolstat(solstatbuf_t *statbuf, const solstat_t *stat)
{
    ...
    gnss_util_trace(4,"addsolstat:\n");
    
    if (statbuf->n>=statbuf->nmax) {
        statbuf->nmax=statbuf->nmax==0?8192:statbuf->nmax*2;
        Sys_Free(statbuf->data);
        if (!(statbuf_data=(solstat_t *)Sys_Malloc(sizeof(solstat_t)*
                                                statbuf->nmax))) {
            gnss_util_trace(1,"addsolstat: memory allocation error\n");
             statbuf->n=statbuf->nmax=0;
            return;
        }
        statbuf->data=statbuf_data;
                           ...
                           solstatbuf_t *statbuf)
{
    ...
    gnss_util_trace(3,"readsolstatdata:\n");
                        ...
                        double tint, solstatbuf_t *statbuf)
{
    ...
    gnss_util_trace(3,"readsolstatt: nfile=%d\n",nfile);
    ...
    for (i=0;i<nfile;i++) {
        ...
        if (!(fp=fopen(path,"r"))) {
            gnss_util_trace(1,"readsolstatt: file open error %s\n",path);
            continue;
        }
        /* read solution status data */
...
extern int readsolstat(char *files[], int nfile, solstatbuf_t *statbuf)
{
    ...
    gnss_util_trace(3,"readsolstat: nfile=%d\n",nfile);
...
extern int outnmea_rmc(unsigned char *buff, const sol_t *sol)
{
    ...
    p+=sprintf(p,"$GPRMC,%02.0f%02.0f%06.3f,A,%02.0f%010.7f,%s,%03.0f%010.7f,%s,%4.2f,%4.2f,%02.0f%02.0f%02d,%.1f,%s,%s",
                   ...
                   const solopt_t *opt)
{
    ...
    if (sol->stat<=SOLQ_NONE||(opt->posf==SOLF_ENU&&asg_norm(rb,3)<=0.0)) {
        return 0;
    }
    ...
    if (opt->timef) time2str(time,s,timeu);
        ...
        if (86400.0*7-gpst<0.5/pow(10.0,timeu)) {
            week++;
            gpst=0.0;
        }
...
* return : none
*-----------------------------------------------------------------------------*/
extern void ag_rtk_solution_out(FILE *fp, const sol_t *sol, const double *rb,
                   const solopt_t *opt)
{
    unsigned char buff[MAXSOLMSG+1];
    int n;
    
    gnss_util_trace(3,"ag_rtk_solution_out  :\n");
