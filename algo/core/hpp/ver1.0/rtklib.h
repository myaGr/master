...
#if defined(PLAYBACK_MODE) && defined(_WIN32)
#ifndef TRACE
#define TRACE     //output rtk trace file when playback on Windows
#endif
#endif

//attention: these two values are set based on the fact that
//the number of channels of 262M is 48
#define CMN_CP_NUM  (35)
#define CMN_OBS_NUM (60)  
#define CMN_CP_NUM_HF (27)
#define CMN_OBS_NUM_HF (40)  
#define MAX_N_WL    (20)
/* switch of system use */
//#define ENAGLO
#define ENACMP
#define ENAGAL
#define ENAQZS
...
#define NC(opt)     ((opt)->adaptSeqEKF==0?0:((NSYSDD) * NF(opt)))
#define NR(opt)     (NP(opt)+NI(opt)+NT(opt)+NL(opt))
#define NX(opt)     (NR(opt)+NC(opt)+NB(opt))

/* state variable index */
#define IRC(i,opt)  (NP(opt)+(i))                 /* receiver clock*/
#define II(s,opt)   (NP(opt)+NC(opt)+(s)-1)                 /* ionos (s:satellite no) */
#define IT(r,opt)   (NP(opt)+NC(opt)+NI(opt)+NT(opt)/2*(r)) /* tropos (r:0=rov,1:ref) */
#define IL(f,opt)   (NP(opt)+NC(opt)+NI(opt)+NT(opt)+(f))   /* receiver h/w bias */
#define IB(s,f,opt) (NR(opt)+NC(opt)+MAXSAT*(f)+(s)-1) /* phase bias (s:satno,f:freq) */

#define FREQ_FIX(f) (1<<(f))
#define FREQ_MASK(f) (1<<(f))

/* constants -----------------------------------------------------------------*/

#define D2R         (PI/180.0)          /* deg to rad */
#define R2D         (180.0/PI)          /* rad to deg */
#define CLIGHT_REC  (1.0/CLIGHT)
...
#define RE_WGS84_POW2 (RE_WGS84*RE_WGS84)
...
#ifndef NFREQ
#define NFREQ       AGGNSS_MAX_FREQ_NUM /* number of carrier frequencies */
#endif
...
#define MAXOBS      64                  /* max number of obs in an epoch, USED IN PEDATAIN, DO NOT CHANGE THIS VALUE! */
#else
#define MAXOBS      64                  /* max number of obs in an epoch, USED IN PEDATAIN, DO NOT CHANGE THIS VALUE! */
#endif
#endif
...
#define MAXRCV      256                 /* max receiver number (1 to MAXRCV) */
#else
...
#define MAXSTRPATH  32                  /* max length of stream path */
...
#define MAXANT      64                  /* max length of station name/antenna type, USED IN PEDATAIN, DO NOT CHANGE THIS VALUE! */
#define MAXSOLBUF   256                 /* max number of solution buffer */
#define MAXOBSBUF   2                   /* max number of observation data buffer */
...
#define MAXMSGNUM   32                  /* max number of msg in msg 1013, USED IN PEDATAIN, DO NOT CHANGE THIS VALUE! */
#define MAXFATBUFF	15					/* max number of cfd buff size*/
...
#define SOLQ_NAME_NONE   "NONE"         /* solution name: no solution */
#define SOLQ_NAME_FIX    "FIX"          /* solution name: fix */
#define SOLQ_NAME_FLOAT  "FLOAT"        /* solution name: float */
#define SOLQ_NAME_SBAS   "SBAS"         /* solution name: SBAS */
#define SOLQ_NAME_DGPS   "DGPS"         /* solution name: DGPS/DGNSS */
#define SOLQ_NAME_SINGLE "SINGLE"       /* solution name: single */
#define SOLQ_NAME_PPP    "PPP"          /* solution name: PPP */
#define SOLQ_NAME_DR     "DR"           /* solution name: dead reconing */
...
#define EPHOPT_BRDC_IODE  6             /* ephemeris option: broadcast ephemeris, need save IODE */
#define EPHOPT_BRDC_TOE  6             /* ephemeris option: broadcast ephemeris, need save IODE */
...
typedef struct {        /* observation data record */
    ...
    int32_t    sys; /* SYS_GPS, SYS_GLO, SYS_GAL... */
...
typedef struct {        /* observation data record */
	...
	float  D[NFREQ+NEXOBS]; /* observation data doppler frequency (Hz) */
	int32_t    sys; /* SYS_GPS, SYS_GLO, SYS_GAL... */
	int32_t    prn; /* svid of each system */
	/* extent field */
	int8_t      glofcn[NFREQ+NEXOBS]; /* glonass fcn, [-7,6], 127 means invalid */
	uint8_t      prStd[NFREQ+NEXOBS];
	uint8_t      cpStd[NFREQ+NEXOBS];
	uint8_t      drStd[NFREQ+NEXOBS];
	uint8_t      trkStat[NFREQ+NEXOBS];
	uint16_t     lockTime[NFREQ+NEXOBS];
} obsd_raw_t;

typedef struct {        /* observation data */
	int n,nmax;         /* number of obervation data/allocated */
	obsd_raw_t *data;       /* observation data records */
	/* extent field */
	int8_t  leapSec;
	uint8_t  recStat;
} obs_raw_t;
...
typedef struct {        /* GLONASS broadcast ephemeris type */
    ...
    int svh,sva,age;    /* satellite health=Bn, accuracy=Ft, age of operation=En */
...
typedef struct {        /* station parameter type */
    ...
    int32_t  antsetup;       /* antenna setup id */
    int32_t  itrf;           /* ITRF realization year */
    int32_t  deltype;        /* antenna delta type (0:enu,1:xyz) */
    double pos[3];      /* station position (ecef) (m) */
    double del[3];      /* antenna position delta (e/n/u or x/y/z) (m) */
    double hgt;         /* antenna height (m) */
} sta_t;

#define QFLAG_PVT_PRRES_GOOD 0x01
#define QFLAG_PVT_KFPOS_GOOD 0x02
#define QFLAG_RTK_PRRES_GOOD 0x04
#define QFLAG_RTK_CPRES_GOOD 0x08
#define QFLAG_RTK_KFPOS_GOOD 0x10
typedef struct {        /* solution type */
    ...
    unsigned char qflag;/* quality flag */
	double post_sigma;
    double fix_post_sigma;
    int fixed_sat_num;
} sol_t;
...
typedef struct {        /* processing options type */
    ...
    double eratio[3];   /* code/phase error ratio */
...
typedef struct {        /* solution options type */
    ...
    int outsln;         /* output solution output (0:no,1:yes) */
...
#define CYCSLIPDET_TIMEGAP  0x01
#define CYCSLIPDET_LLI      0x02
#define CYCSLIPDET_CN0      0x04
#define CYCSLIPDET_DOPPLER  0x08
#define CYCSLIPDET_HODIFF   0x10
#define CYCSLIPDET_SDSAT    0x20
#define CYCSLIPDET_TDRES    0x40
#define CYCSLIPDET_TDEST    0x80
#define CYCSLIPDET_CARRIER 0x100
#define CYCSLIPDET_LLI_HC  0x200
#define CYCSLIPDET_ZD_MW   0x400
#define CYCSLIPDET_ZD_GF   0x800
#define CYCSLIPDET_DDSAT  0x1000
#define CYCSLIPDET_CPDR   0x2000
#define CYCSLIPDET_LOCKSAT 0x4000

#define TD_CHECK_RES    0x01
#define TD_CHECK_EST    0x02
#define DD_CHECK_REFSAT 0x04

/* for sat use control */
#define SATCTRL_ADJQ    0x01
#define SATCTRL_REJC    0x02
#define SATCTRL_DWTC    0x04
#define SATCTRL_REJP    0x08
#define SATCTRL_DWTP    0x10

//Scenario
#define SCE_OPENSKY       0
#define SCE_SEMISKY       1
#define SCE_CLOSESKY      2

typedef struct {        /* sat use control */
	uint8_t      flag;
	//uint8_t      qambf;
	//uint8_t      dwtcf;
	//uint8_t      dwtpf;
	uint8_t      canberef;
	uint8_t      fix_rejflag;        /* flag about fix */
	uint8_t      halfflag;
	uint8_t      rqflag[NFREQ];             /* rover quality */
	uint8_t      avalbl[2];  //0:cp,1:pr
	uint16_t     pr_qoschck[NFREQ];         /* rover pr qos check mask */
	uint16_t     dr_qoschck[NFREQ];         /* rover dr qos check mask */
	float     pr_diff;            /* rover pr diff from spp */
	float     dr_diff;            /* rover dr diff from spp */
} satctrl_t;

typedef struct {        /* obs for only L1 */
	gtime_t time;       /* receiver sampling time (GPST) */
	unsigned char cn0;      /* signal strength (dBHz) */
	int    llicnt;          /* loss of lock indicator */
	double L[6]; /* carrier-phase (cycle), [0]:raw, [1]:1st diff ,[2]:2nd diff, ... */
	double P;    /* observation data pseudorange (m) */
	float  D;    /* observation data doppler frequency (Hz) */
} obs_l1;

typedef struct {        /* obs for only L1 */
	double  y[NFREQ*2];   /* carrier phase zdres */
	//double  e[3];       /* dcos */
} td_l1;

typedef struct {        /* hod info for one freq */
	obs_l1  obs[2][MAXOBSBUF];  /* obs store (0:rov  1: ref) */
	unsigned int obsn[2];       /* valid obs buffer count */
} hod_l1;

#define LC_SLIP_CHECK_MW  0x01
#define LC_SLIP_CHECK_GF  0x02
typedef struct {        /* linear combination for multi-freq slip det */
	uint8_t     lc_flag;
	int32_t    mw_cnt;
	//float  mw_tor;
	float  mw_dNw;
...
typedef struct {
	uint32_t    iode;
	float  toe[2]; /*1: last, 0: current toe*/
	float  slip[NFREQ];
} slpdet_satdd_t;
...
typedef struct {        /* satellite status type */
	...
	unsigned char ambfix[NFREQ];
	unsigned char present[2];   /* if present in this epoch */
    uint8_t      tdcheck[NFREQ];     /* TD_CHECK_RES|TD_CHECK_EST */
	uint8_t     freq_valid;  /* bit0:freq1 bit1:freq2 bit2:freq3 */
    int16_t   slipflag[2][NFREQ];   /* CYCSLIPDET_XXXX|CYCSLIPDET_XXXX */
	...
	double  tdy[NFREQ];         /* td y value */
	double  tdres[NFREQ];       /* td post-residual */
	double sdamb[NFREQ];
	gtime_t pt[2][NFREQ]; /* previous carrier-phase time */
    slpdet_lc_t slp_lc[2];      /* linear combination for slip det */
    slpdet_satdd_t slp_ref;
	td_l1   td[2];              /* for td cycle detection */
	satctrl_t usectrl;          /* adjust sat Q and R */
#ifdef USED_IN_MC262M
	uint8_t     trkstate[NFREQ];
	U16    lockms[2][NFREQ];
	float  cp_std[NFREQ];
	float  pr_std[NFREQ];
    ...
    /* for cycle detect */
    //hod_l1  hod[NFREQ];
    //S16   pslipflag[2][NFREQ];  /* last slip flag */
	//double  gf2;        /* geometry-free phase L1-L5 (m) */
} ssat_t;

#define RTK_NOROVROBS     0x01
#define RTK_NOROVRSPP     0x02
#define RTK_NOBASEOBS     0x04
#define RTK_NOBASEPOS     0x08
#define RTK_DIFAGEEXPIRE  0x10
#define RTK_BASECHANGE    0x20
...
/* KF status for RTK */
#define RTK_KF_IDLE         0x00  /* KF has done nothing */
#define RTK_KF_INIT         0x01  /* KF is to init by spp results */
#define RTK_KF_RESET        0x02  /* KF is to reset pva and amb */
#define RTK_KF_RESET_AMB    0x04  /* KF is to reset amb only */
#define RTK_KF_RESTART      0x08  /* KF is to restart */
#define RTK_KF_RUN          0x10  /* KF is running */
#define RTK_KF_ENLARGEQ     0x20  /* KF is to enlarge Q while KF is running */

#define DDRES_SINGLE   0
#define DDRES_PREDICT  1
#define DDRES_UPDATE   2
#define DDRES_FIXED    3
#if defined(PLAYBACK_MODE)
#define DDRES_HPREF    4

#define MAX_DDRES_NUM  5
#else
#define MAX_DDRES_NUM  4
#endif

#define DDRESFLAG_CP_GOOD     0x01
#define DDRESFLAG_PR_GOOD     0x02
#define DDRESFLAG_RMSCPVALID  0x04
#define DDRESFLAG_RMSPRVALID  0x08
#define DDRESFLAG_CHECKVALID  0x10
#define DDRESFLAG_REJTOOMUCH  0x20
#define DDRESFLAG_RTD_DRONLY  0x40

#define UBX_LOCKMS_THRES 250

#define Part_LAMBDA_ALL      0
#define Part_LAMBDA_ELECN0   1
#define Part_LAMBDA_DDAMB    2

typedef struct {
	unsigned char rejc;  /* rejected cp count */
	unsigned char rejp;  /* rejected pr count */
	unsigned char nc;    /* valid cp count */
	unsigned char np;    /* valid pr count */
	unsigned char cntc;  /* good cp count */
	unsigned char cntp;  /* good pr count */
	unsigned char ncmad;  /* mad cp count */
	unsigned char npmad;  /* mad pr count */
	unsigned char flag;
	float rmsc,stdc;
	float rmsp,stdp;
	float rmsfc,rmsfp;
...
#define SHIFT_FLAG_FLOAT2FIX     0x01
#define SHIFT_FLAG_SMALLSHIFT    0x02
#define SHIFT_FLAG_LARGESHIFT    0x04
#define SHIFT_FLAG_NO_FEEDBACK   0x08
typedef struct {
	uint32_t      flag;
	uint8_t       maxadjQcnt;  /* max count for enlarge Q */
	uint8_t       adjQcnt;     /* count for enlarge Q */
	int8_t       adjPcnt;     /* count for enlarge P */
	float    Qfact;       /* enlarge factor for Q */
	float    spplsvel[MAX_SOL_BUF];
	float    sppkfvel[MAX_SOL_BUF];
	float    sppheading[MAX_SOL_BUF];  /* spp kf heading */
	float    sppposrms[MAX_SOL_BUF];   /* spp kf pos rms */
	float    rtkvel[MAX_SOL_BUF];      /* rtk kf vel */
	float    rtkheading[MAX_SOL_BUF];  /* rtk kf heading */
	float    dis2D[MAX_SOL_BUF];
	float    discurpre[MAX_SOL_BUF];   /*RTK dis3d between current sol and pre sol*/
	uint8_t       sppkfvsat;   /* valid sat in spp kf */
	uint8_t		 feedback_flag;
	uint8_t		 last_feedback_flag;
	uint64_t      sollist;     /* sol valid flag */
	uint64_t      sppsollist;     /* spp sol valid flag */
	uint64_t      dis2dList1;  /* dis2d < 5.0 */
	uint64_t      dis2dList2;  /* dis2d > 15.0 */
	uint64_t      fixlist;     /* 0:nofix   1:fix */
	int32_t      nofixc;
	int32_t      nofixcnt;
	int32_t      flo2fixwait;
	double   prexyz[3];
	double	 feedback_rr[3];
} shiftdet_t;

typedef struct{
	double horicfd[3];	/*horizontal confident bounder 0.683 0.955 0.997*/
	double upcfd[3];	/*up confident bounder 0.683 0.955 0.997*/
	double phcfd[3];	/*previous fix cfd*/
	double pucfd[3];
 } confident_t;
...
typedef struct {
	...
	double avecn0[2];
	double sigmap[2];
	double sigmac[2];	/*code sigma*/
	double sigmad;
} rtkfactor_t;

typedef struct {
	unsigned char cfdflag;  /*rtk cfd valid flag*/
	double horicfd[3];	/*horizontal confident bounder 0.683 0.955 0.997*/
	double upcfd[3];	/*up confident bounder 0.683 0.955 0.997*/
	double phcfd[3];	/*previous fix cfd*/
	double pucfd[3];
...
typedef struct {
	...
	uint8_t preuncconf;
	double prepos[3];
	float pos_diff_h[MAXUNCBUFF];
	float pos_diff_v[MAXUNCBUFF];
}prediff_t;

typedef struct {
	uint8_t vflag;
	uint8_t uncnum;
	uint8_t latest_fix_satn;
	uint8_t state;
	uint8_t prefixf;
	double hunc;
	double upunc; //the up uncentainty 
	double latest_fix_hunc;
	double latest_fix_upunc;
	double hunc_smt;
	double upunc_smt;
	double hunc_buff[MAXUNCBUFF];
	rtkfactor_t latest_fix_fat;
	uncsmt_t huncsmt;
	uncsmt_t vuncsmt;
	prediff_t prediff;
}uncertainty_t;
...
typedef struct {
	uint64_t     flag;           /*solution flag*/
	int     nb;             /*amb of wl fixed*/
	double  sol[3];         /*wl sol*/
	double  ratio;          /*ratio of wl amb fixed*/
	int     isat[MAX_N_WL]; /*wl fixed sat include reference satellite at the first*/
	int     nwlsys[NSYSDD]; /*the wl number of each system*/
	double  bias[MAX_N_WL]; /*fixed wl amb*/
	double  adop;
	double  pdop;
	double  sigma;
}wl_t;
...
typedef struct {        /* RTK control/result type */
    ...
    int* ix;            /* index of float states*/
    double *x, *P;      /* float states and their covariance */
    double *xa,*Pa;     /* fixed states and their covariance */
    int nfix;           /* number of continuous fixes of ambiguity */
    int nbias;          /* number of fixed bias */
    ssat_t* ssat[MAXSAT]; /* satellite status */
    prcopt_t opt;       /* processing options */
    int rbflag;         /* rb valid flag */
    double rbspp[3];        /* spp results while rb ud failed */
    unsigned char refs[NSYSDD];  /* m=0:gps/qzs/sbs,1:glo,2:gal,3:bds */
	int nrefs;           /*number of reference sat changed*/
    gtime_t pt[2];       /* last td obs time, 0:rov, 1:ref */
    unsigned char vsat[MAXOBS];  /* last valid sat list */
    unsigned char nsat; /* last valid sat count */
    double prr[3];      /* predict rr of last epoch, which was used to get y */
    double td_dx[3];    /* delta X between cuurent pos and last pos */
    double tdsigma;     /* td post-sigma0 of ls */
    double ptor;        /* rover previous timestamp (s) */
	double kf_tor;
    unsigned char rtkevent; /* event which will affect rtk flow */
    /* filter */
    unsigned char kfstatus; /* RTK_KF_INIT,RTK_KF_RUN,... */
    unsigned char kfextpol; /* time update only */
    unsigned char kfextpcnt;/* extrapolate count */
    int           kfcnt;    /* count for continuous KF_RUN count */
    int           kfstartwait; /* count for kf start wait */
    double *F;
    double *Q;
	double *R;
    double updFQtt;
    uint64_t qcpasscnt;
    uint64_t closkycnt;
	uint64_t fixstateseq;
	uint64_t druseseq;
	uint64_t prefatstate;
	int fatstatecnt;
    unsigned char llisatcnt[4];/* 0:gps, 1:glo, 2:bds, 3:gal */
    unsigned char satcnt[4];   /* 0:gps, 1:glo, 2:bds, 3:gal */
	unsigned char pvtsatcnt[4];/* 0:gps, 1:glo, 2:bds, 3:gal */
    unsigned char nollicnt;    /* all no lli mark count */
    ddres_t dd_res[MAX_DDRES_NUM]; /* ddres type */
    shiftdet_t    shift;       /* shift detection */
    staticsol_t   staticsol;
    unsigned char biasnum[NSYSDD];		/*fixed bias each sys*/
    gtime_t prefixt;		/*previous fix time*/
    double prefixr;		    /*previous fix ratio*/
    unsigned char prefixn;  /*previous fix amb num*/
    unsigned char prefix_sat; /* prefix satcnt */
    unsigned char fixstate[MAXSAT];	/*fix state*/
    unsigned char prefixf;		    /*prefix valid flag for current epoch*/
	unsigned char prefloat;
    double prefixsol[3];	        /*prefix sol*/
    double prefixsigma0;            /*prefix sigma0*/
    float  prefixQmat[6];           /*prefix var-cov matrix*/
    rtkfactor_t fat;
    uint8_t rtduseflag;
    uint8_t cp_pr_cnt;
#ifdef USED_IN_MC262M
	uint8_t lockvs[NSYSDD];
#endif
	uint8_t dewNum_pr;
	uint8_t dewNum_cr;
#ifdef USED_IN_MC262M
	...
	confidence_t cfd;
	uncertainty_t unc;
	unsigned char noSkipCnt;   /* all no circle skip mark count */
	uint8_t trstCloCnt;   /*transient close sky counter*/
	double Qratio;
	wl_t wl;
	int ambfix_flag;
	int VPT_cn0;
...
typedef struct {        /* receiver raw data control type */
    ...
    obs_raw_t obs;      /* raw observation data */
	...
	sol_pvt_t pvt;      /* QX extension: instore receiver PVT result. */
} raw_t;
...
#define mat(n,m) asg_mat(n,m,__FUNCTION__,__LINE__,__FILE__)
EXPORT double* asg_mat(int n, int m, const char* func, long line, const char* file);

#define imat(n,m) asg_imat(n,m,__FUNCTION__,__LINE__,__FILE__)
EXPORT int* asg_imat (int n, int m, const char* func, long line, const char* file);

#define zeros(n,m) asg_zeros(n,m,__FUNCTION__,__LINE__,__FILE__)
EXPORT double *asg_zeros(int n, int m, const char* func, long line, const char* file);

EXPORT double *eye  (int n);
EXPORT double dot (const double *a, const double *b, int n);
EXPORT double asg_norm(const double *a, int n);
...
EXPORT void matmulsm(const char *tr, int n, int k, int m, double alpha,
	const double *A, const double *B, double beta, double *C);
EXPORT void matmul_flt(const char *tr, int n, int k, int m, double alpha,
				   const float *A, const float *B, double beta, float *C);
EXPORT int  matinv(double *A, int n);
EXPORT int  matinvsm(double *A, int n);
...
EXPORT int  lsq   (const double *A, const double *y, int n, int m, double *x,
                   double *Q);
EXPORT int  ag_rtk_kf_filter(double *x, double *P, const double *H, const double *v,
...
EXPORT void mat_screen_print(const double *A, int n, int m);
...
EXPORT gtime_t gnss_epoch2time(const double *ep);
EXPORT void    gnss_time2epoch(gtime_t t, double *ep);
EXPORT gtime_t gnss_gpst2time(int week, double sec);
...
EXPORT gtime_t gln2utc(int NT, int N4, double tod);
EXPORT void    utc2gln(int* NT, int* N4, double* tod, double* p_utc);
...
#ifdef TRACE
void gnss_util_traceopen(const char *file);
void gnss_util_traceclose(void);
void gnss_util_tracelevel(int level);
void gnss_util_trace(int level, const char *format, ...);
void gnss_util_tracet   (int level, const char *format, ...);
void gnss_util_tracemat (int level, const double *A, int n, int m, int p, int q);
void gnss_util_traceobs (int level, const obsd_t *obs, int n);
void gnss_util_tracenav (int level, const nav_t *nav);
void gnss_util_tracegnav(int level, const nav_t *nav);
void gnss_util_tracehnav(int level, const nav_t *nav);
void gnss_util_tracepeph(int level, const nav_t *nav);
void gnss_util_tracepclk(int level, const nav_t *nav);
void gnss_util_traceb   (int level, const unsigned char *p, int n);
#else
#define gnss_util_traceopen(file)     {}
#define gnss_util_traceclose()        {}
#define gnss_util_tracelevel(level)   {}
#define gnss_util_trace(level, ...)   {}
#define gnss_util_tracet(level, ...)  {}
#define gnss_util_tracemat(level, A, n, m, p, q) {}
#define gnss_util_traceobs(level, obs, n) {}
#define gnss_util_tracenav(level, nav)    {}
#define gnss_util_tracegnav(level, nav)   {}
#define gnss_util_tracehnav(level, nav)   {}
#define gnss_util_tracepeph(level, nav)   {}
#define gnss_util_tracepclk(level, nav)   {}
#define gnss_util_traceb(level, p, n)     {}
#endif
...
extern double ag_getwavelen(int sys, int frq, int fcn);
...
EXPORT int readrnxt(const char *file, int rcv, gtime_t ts, gtime_t te,
                    ...
                    sta_t *sta);
EXPORT extern int readrnxt_multibase(const char *file, int rcv,
    double tint, const char *opt, obs_t *obs, nav_t *nav, sta_t *sta, ppk_base_swtich_t *p_switch);
...
EXPORT void*  selephfromnav(gtime_t teph, int sat, const nav_t *nav, int iode);
...
EXPORT int  satpos(gtime_t time, gtime_t teph, int sat, int ephopt,
                   ...
                   int *svh);
EXPORT void ag_gnss_satposs(gtime_t time, const obsd_t *obs, int n, const nav_t *nav,
                    int sateph, double *rs, double *dts, double *var, int *svh, double *azel);
...
#define ID_NAVPVT   0x0107      /* ubx message id: nav pvt solution */
...
#define ID_ESFRAW   0x1003      /* ubx message id: esf raw data */
#define ID_ESFMEAS  0x1002      /* ubx message id: esf meas data */

#define UBX_ERR_CHECKSUM       (-1)
#define UBX_ERR_LENTH          (-2)

//EXPORT unsigned int asg_getbitu(const unsigned char *buff, int pos, int len);
//EXPORT int          asg_getbits(const unsigned char *buff, int pos, int len);
...
EXPORT void ag_rtk_solution_out(FILE *fp, const sol_t *sol, const double *rb,
                     const solopt_t *opt);
...
EXPORT void resetsysopts_lite(prcopt_t *popt);
EXPORT void getsysopts_lite(prcopt_t *popt);
...
EXPORT int ag_gnss_pntpos(const obsd_t *obs, int n, const nav_t *nav,
                  const prcopt_t *opt, sol_t *sol, double *azel,
                  ssat_t *ssat, char *msg);

/* precise positioning -------------------------------------------------------*/
EXPORT void ag_rtk_init(rtk_t *rtk, const prcopt_t *opt);
EXPORT void ag_rtk_free(rtk_t *rtk);
EXPORT int  ag_rtk_position(rtk_t *rtk, const obsd_t *obs, int nobs, const nav_t *nav);
...
extern int post_preprocess(int reverse);
extern int post_readrnx(char ** infile, const int *index, int n,
	prcopt_t *prcopt, ppk_base_swtich_t *p_switch);
extern double* post_get_base_pos(prcopt_t *prcopt, int base_idx);
extern void post_free_sol(void);
extern int post_sol_percentage(int reverse);
...
extern int asg_init_raw(raw_t *raw);
extern void asg_free_raw(raw_t *raw);

unsigned int asg_getbitu(const unsigned char* buff, int pos, int len);
int asg_getbits(const unsigned char* buff, int pos, int len);
#ifdef ASM_MATMUL
extern void Fast_Rtk_matmul(const char* tr, int n, int k, int m, double alpha,
    const double* A, const double* B, double beta, double* C);
#endif
#ifdef __cplusplus
}
