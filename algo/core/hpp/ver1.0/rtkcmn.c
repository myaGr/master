...
*           2009/04/10 1.9 add function adjgpsweek(),asg_getbits(),asg_getbitu()
...
*           2011/05/28 1.16 fix bug on half-hour offset by gnss_time2epoch()
*                           added api:
*                               uniqnav()
...
const prcopt_t prcopt_default={ /* defaults processing options */
    ...
    0,3,0,1,                    /* sateph,modear,glomodear,bdsmodear */
    5,0,2,1,                   /* maxout,minlock,minfix,armaxiter */
    ...
    {30.0,0.03,0.3},            /* std[] */
    {1E-4,1E-3,1E-4,1E1,1E1,0.0}, /* prn[] */
    5E-12,                      /* sclkstab */
    {2.5,0.9999,0.25,0.1,0.05}, /* thresar */
    0.0,0.0,0.05,          /* elmaskar,almaskhold,thresslip */
    60.0,30.0,30.0,             /* maxtdif,maxinno,maxgdop */
    {0},{0},{0},                /* baseline,ru,rb */
    {"",""},                    /* anttype */
    {{0}},{{0}},{0},            /* antdel,pcv,exsats */
    0,0,1                       /* maxaveep,initrst,outsingle */
};
const solopt_t solopt_default={ /* defaults solution output options */
    SOLF_LLH,TIMES_GPST,1,3,    /* posf,times,timef,timeu */
    0,0,1,0,0,0,0,              /* degf,outsln,outhead,outopt,datum,height,geoid */
...
char *obscodes[]={       /* observation code strings */
...
char codepris[7][MAXFREQ][16] = {  /* code priority table */
   ...
   /* L1/E1      L2/B1        L5/E5a/L3 L6/LEX/B3 E5b/B2    E5(a+b)  S */
	{"CPYWMNSL","QPWYCMNDSLX","WIXQ"     ,""       ,""       ,""      ,""    }, /* GPS */
	{"PC"      ,"PC"        ,"IQX"     ,""       ,""       ,""      ,""    }, /* GLO */
	{"CABXZ"   ,"Q"          ,"IQX"     ,"ABCXZ"  ,"IQX"    ,"IQX"   ,""    }, /* GAL */
	{"CSLXZ"   ,"QSLX"       ,"IQX"     ,"SLX"    ,""       ,""      ,""    }, /* QZS */
	{"C"       ,""          ,"IQX"     ,""       ,""       ,""      ,""    }, /* SBS */
	{"IQX"     ,"NIQX"       ,"IQX"     ,"IQX"    ,"IQX"    ,""      ,""    }, /* BDS */
	{""        ,""          ,"ABCX"    ,""       ,""       ,""      ,"ABCX"}  /* IRN */
};
...
extern int satexclude(int sat, int svh, const prcopt_t *opt)
{
    ...
    if (svh) {
        gnss_util_trace(3,"unhealthy satellite: sat=%3d svh=%02X\n",sat,svh);
        return 1;
    }
    return 0;
}
...
{
    gnss_util_trace(3,"setcodepri:sys=%d freq=%d pri=%s\n",sys,freq,pri);
...
*-----------------------------------------------------------------------------*/
extern unsigned int asg_getbitu(const unsigned char *buff, int pos, int len)
{
    unsigned int bits=0;
    int i;
...
extern int asg_getbits(const unsigned char *buff, int pos, int len)
{
...
extern unsigned int rtk_crc32(const unsigned char *buff, int len)
{
    ...
    gnss_util_trace(4,"rtk_crc32: len=%d\n",len);
...
extern unsigned int rtk_crc24q(const unsigned char *buff, int len)
{
    ...
    gnss_util_trace(4,"rtk_crc24q: len=%d\n",len);
...
extern unsigned short rtk_crc16(const unsigned char *buff, int len)
{
    ...
    gnss_util_trace(4,"rtk_crc16: len=%d\n",len);
...
extern int decode_word(unsigned int word, unsigned char *data)
{
    ...
    gnss_util_trace(5,"decodeword: word=%08x\n",word);
...
*-----------------------------------------------------------------------------*/
extern double *asg_mat(int n, int m, const char* func, long line, const char* file)
{
	double *p;
	...
	p = (double*)Sys_Calloc(1, sizeof(double) * n * m);
    if (!p)
    {
        return NULL;
    }
...
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
extern int* asg_imat(int n, int m, const char* func, long line, const char* file)
{
	int *p;
	...
	p = (int*)Sys_Calloc(1, sizeof(int) * n * m);
    if (!p)
    {
        return NULL;
    }
...
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
extern double *asg_zeros(int n, int m, const char* func, long line, const char* file)
{
	double *p;
...
/* euclid asg_norm -----------------------------------------------------------------
* euclid asg_norm of vector
* args   : double *a        I   vector a (n x 1)
*          int    n         I   size of vector a
* return : || a ||
*-----------------------------------------------------------------------------*/
extern double asg_norm(const double *a, int n)
{
...
{
    double r;
    if ((r=asg_norm(a,3))<=0.0) return 0;
    b[0]=a[0]/r;
    b[1]=a[1]/r;
    b[2]=a[2]/r;
    return 1;
		...
		case 1:
			for (i = 0; i < n; i++)
			{
				for (j = 0; j < k; j++)
				{
					...
					if (beta == 0.0) C[t] = alpha * d; else C[t] = alpha * d + beta * C[t];
				}
			}
			break;
		case 2:
...
extern void matmulsm(const char *tr, int n, int k, int m, double alpha,
	const double *A, const double *B, double beta, double *C)
{
	double d;
	int i, j, x, f = tr[0] == 'N' ? (tr[1] == 'N' ? 1 : 2) : (tr[1] == 'N' ? 3 : 4);
	...
	{
	case 1:
		if (tr[2] == '1'&&tr[3] == '0'&&tr[4] == '0'&&beta==0.0)
		{
			for (i = 0; i < n; i++)
			{
				for (j = 0; j < k; j++)
		...
		else if(tr[2] == '0'&&tr[3] == '1'&&tr[4] == '1'&&beta == 0.0)
		{
			for (i = 0; i < n; i++)
			{
				for (j = 0; j < k; j++)
		...
		else
		{
			for (i = 0; i < n; i++)
			{
				for (j = 0; j < k; j++)
				{
					...
					for (x = 0; x < m; x++)
					{
						d += (tr[2] == '1' ? A[ISM(i, x)] : A[i + x * n])*(tr[3] == '1' ? B[ISM(x, j)] : B[x + j * m]);
					}

					if (beta == 0.0) {
						if (tr[4] == '1') C[ISM(i, j)] = alpha * d;
						else C[i + j * n] = alpha * d;
					}
					else {
						if (tr[4] == '1') C[ISM(i, j)] = alpha * d + beta * C[ISM(i, j)];
	...
	case 2:
		for (i = 0; i < n; i++)
		{
			for (j = 0; j < k; j++)
			{
				...
				for (x = 0; x < m; x++)
				{
					d += (tr[2] == '1' ? A[ISM(i, x)] : A[i + x * n])*(tr[3] == '1' ? B[ISM(j, x)] : B[j + x * k]);
				}
	...
	case 3:
		for (i = 0; i < n; i++)
		{
			for (j = 0; j < k; j++)
			{
				...
				for (x = 0; x < m; x++)
				{
					d += (tr[2] == '1' ? A[ISM(x, i)] : A[x + i * m])*(tr[3] == '1' ? B[ISM(x, j)] : B[x + j * m]);
				}
	...
	case 4:
		for (i = 0; i < n; i++)
		{
			for (j = 0; j < k; j++)
			{
				...
				for (x = 0; x < m; x++)
				{
					d += (tr[2] == '1' ? A[ISM(x, i)] : A[x + i * m])*(tr[3] == '1' ? B[ISM(j, x)] : B[j + x * k]);
				}
	...
	//	d = 0.0;
	//	switch (f) {
	//		case 1: for (x = 0; x < m; x++) d += (tr[2] == '1' ? A[ISM(i, x)] : A[i + x * n])*(tr[3] == '1' ? B[ISM(x, j)] : B[x + j * m]); break;
	//		case 2: for (x = 0; x < m; x++) d += (tr[2] == '1' ? A[ISM(i, x)] : A[i + x * n])*(tr[3] == '1' ? B[ISM(j, x)] : B[j + x * k]); break;
	//		case 3: for (x = 0; x < m; x++) d += (tr[2] == '1' ? A[ISM(x, i)] : A[x + i * m])*(tr[3] == '1' ? B[ISM(x, j)] : B[x + j * m]); break;
	//		case 4: for (x = 0; x < m; x++) d += (tr[2] == '1' ? A[ISM(x, i)] : A[x + i * m])*(tr[3] == '1' ? B[ISM(j, x)] : B[j + x * k]); break;
	//	}
...
extern void matmul_flt(const char *tr, int n, int k, int m, double alpha,
	const float *A, const float *B, double beta, float *C)
{
	...
	for (i = 0; i < n; i++) for (j = 0; j < k; j++) {
		d = 0.0;
		switch (f) {
		case 1: for (x = 0; x < m; x++) d += (double)A[i + x * n] * B[x + j * m]; break;
		case 2: for (x = 0; x < m; x++) d += (double)A[i + x * n] * B[j + x * k]; break;
		case 3: for (x = 0; x < m; x++) d += (double)A[x + i * m] * B[x + j * m]; break;
		case 4: for (x = 0; x < m; x++) d += (double)A[x + i * m] * B[j + x * k]; break;
		}
		if (beta == 0.0) C[i + j * n] = (float)(alpha * d); else C[i + j * n] = (float)(alpha * d + beta * C[i + j * n]);
	}
}
...
static int ludcmp(double *A, int n, int *indx, double *d)
{
    ...
    *d=1.0;
    for (i=0;i<n;i++) {
        ...
        if (big>0.0) vv[i]=1.0/big; else {Sys_Free(vv); return -1;}
    }
    for (j=0;j<n;j++) {
        for (i=0;i<j;i++) {
        ...
        indx[j]=imax;
        if (A[j+j*n]==0.0) {Sys_Free(vv); return -1;}
        if (j!=n-1) {
...
/* inverse of symmetric positive matrix ---------------------------------------------------------*/
extern int matinvsm(double *A, int n)
{
	int i, j, k, m;
	double w, g, *b;
...
#else
        Fast_Rtk_matmul(tr[0] == 'N' ? "NN" : "TN", n, m, n, 1.0, B, Y, 0.0, X);
#endif
    }
    Sys_Free(B);
    return info;
               ...
               double *Q)
{
    ...
    if (!(info=matinvsm(Q,n))) matmul("NN",n,1,n,1.0,Q,Ay,0.0,x); /* x=Q^-1*Ay */
#else
    Fast_Rtk_matmul("NN", n, 1, m, 1.0, A, y, 0.0, Ay); /* Ay=A*y */
    Fast_Rtk_matmul("NT", n, n, m, 1.0, A, A, 0.0, Q);  /* Q=A*A' */
    if (!(info = matinvsm(Q, n))) Fast_Rtk_matmul("NN", n, 1, n, 1.0, Q, Ay, 0.0, x); /* x=Q^-1*Ay */
#endif
    Sys_Free(Ay);
    return info;
}
...
#else
    Fast_Rtk_matmul("NN", n, m, n, 1.0, P, H, 0.0, F);       /* Q=H'*P*H+R */
    Fast_Rtk_matmul("TN", m, m, n, 1.0, H, F, 1.0, Q);
    if (!(info = matinv(Q, m))) {
        Fast_Rtk_matmul("NN", n, m, m, 1.0, F, Q, 0.0, K);   /* K=P*H*Q^-1 */
        Fast_Rtk_matmul("NN", n, 1, m, 1.0, K, v, 1.0, xp);  /* xp=x+K*v */
        Fast_Rtk_matmul("NT", n, n, m, -1.0, K, H, 1.0, I);  /* Pp=(I-K*H')*P */
        Fast_Rtk_matmul("NN", n, n, n, 1.0, I, P, 0.0, Pp);
    }
#endif
    Sys_Free(F); Sys_Free(Q); Sys_Free(K); Sys_Free(I);
    return info;
}
extern int ag_rtk_kf_filter(double *x, double *P, const double *H, const double *v, double *R, int n, int m)
{
	double *F_, *K_, *I_, *Pp_;
	int info;

	F_ = mat(n, m); 
	matmulsm("NN100", n, m, n, 1.0, P, H, 0.0, F_);
#ifndef ASM_MATMUL
	matmul("TN", m, m, n, 1.0, H, F_, 1.0, R);	/* Q=H'*P*H+R */
#else
    Fast_Rtk_matmul("TN", m, m, n, 1.0, H, F_, 1.0, R);	/* Q=H'*P*H+R */
#endif
	info = matinvsm(R, m);

	if (!info) {
#ifndef ASM_MATMUL
		K_ = mat(n, m); matmul("NN", n, m, m, 1.0, F_, R, 0.0, K_);	/* K=P*H*Q^-1 */
		Sys_Free(F_);
		matmul("NN", n, 1, m, 1.0, K_, v, 1.0, x);		/* xp=x+K*v */
		I_ = eye(n); matmul("NT", n, n, m, -1.0, K_, H, 1.0, I_);
		Sys_Free(K_);
		Pp_ = mat(1, NSM(n)); matmulsm("NN011", n, n, n, 1.0, I_, P, 0.0, Pp_);	/* Pp=(I-K*H')*P */
		Sys_Free(I_);
#else
        K_ = mat(n, m); Fast_Rtk_matmul("NN", n, m, m, 1.0, F_, R, 0.0, K_);	/* K=P*H*Q^-1 */
        Sys_Free(F_);
        Fast_Rtk_matmul("NN", n, 1, m, 1.0, K_, v, 1.0, x);		/* xp=x+K*v */
        I_ = eye(n); Fast_Rtk_matmul("NT", n, n, m, -1.0, K_, H, 1.0, I_);
        Sys_Free(K_);
        Pp_ = mat(1, NSM(n)); matmulsm("NN011", n, n, n, 1.0, I_, P, 0.0, Pp_);	/* Pp=(I-K*H')*P */
        Sys_Free(I_);
#endif
...
#else
            Fast_Rtk_matmul("NN", n, 1, n, 1.0, invQf, xf, 0.0, xx);
            Fast_Rtk_matmul("NN", n, 1, n, 1.0, invQb, xb, 1.0, xx);
            Fast_Rtk_matmul("NN", n, 1, n, 1.0, Qs, xx, 0.0, xs);
#endif
        }
    }
    Sys_Free(invQf); Sys_Free(invQb); Sys_Free(xx);
    return info;
}
extern void mat_screen_print(const double *A, int n, int m)
{
	int i, j;
	printf("\n");
	for (i = 0; i < n; i++) {
		for (j = 0; j < m; j++) printf("%.1f  ", A[j + i * m]);
		printf("\n");
	}
	printf("\n");
}
...
*-----------------------------------------------------------------------------*/
extern gtime_t gnss_epoch2time(const double *ep)
{
...
* notes  : proper in 1970-2037 or 1970-2099 (64bit time_t)
*-----------------------------------------------------------------------------*/
extern void gnss_time2epoch(gtime_t t, double *ep)
{
...
* return : gtime_t struct
*-----------------------------------------------------------------------------*/
extern gtime_t gnss_gpst2time(int week, double sec)
{
    gtime_t t= gnss_epoch2time(gpst0);
...
{
    gtime_t t0= gnss_epoch2time(gpst0);
...
{
    gtime_t t= gnss_epoch2time(gst0);
...
{
    gtime_t t0= gnss_epoch2time(gst0);
    time_t sec=t.time-t0.time;
    int w=(int)(sec/(86400*7));
...
{
    gtime_t t= gnss_epoch2time(bdt0);
...
{
    gtime_t t0= gnss_epoch2time(bdt0);
    time_t sec=t.time-t0.time;
    int w=(int)(sec/(86400*7));
...
extern gtime_t timeget_old_ver(void)
{
    gtime_t time = {0};
    double ep[6]={0};
...
* !notice : this function is re-defined by gnss_common.c
*-----------------------------------------------------------------------------*/
/*extern gtime_t gpst2utc(gtime_t t)
{
    ...
    for (i=0;leaps[i][0]>0;i++) {
        ...
        if (timediff(tu,gnss_epoch2time(leaps[i]))>=0.0) return tu;
    }
    return t;
}*/
/* utc to gpstime --------------------------------------------------------------
...
/*extern gtime_t utc2gpst(gtime_t t)
{
    ...
    for (i=0;leaps[i][0]>0;i++) {
        if (timediff(t,gnss_epoch2time(leaps[i]))>=0.0) return timeadd(t,-leaps[i][6]);
    }
    return t;
}*/
/* gpstime to bdt --------------------------------------------------------------
...
{
    double ep[6],sec;
    gnss_time2epoch(time,ep);
    sec=ep[3]*3600.0+ep[4]*60.0+ep[5];
    ep[3]=ep[4]=ep[5]=0.0;
    *day= gnss_epoch2time(ep);
    return sec;
}
/* utc to gmst -----------------------------------------------------------------
...
extern double utc2gmst(gtime_t t, double ut1_utc)
{
    ...
    t1=timediff(tut0, gnss_epoch2time(ep2000))/86400.0/36525.0;
    t2=t1*t1; t3=t2*t1;
...
extern double time2doy(gtime_t t)
{
    ...
    return timediff(t, gnss_epoch2time(ep))/86400.0+1.0;
}

static int leap_year_days_list[12] = { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
static int days_list[12] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
/*  ---------------------------------------------------------
* 
* args   : 
* return : 
*-----------------------------------------------------------------------------*/
gtime_t gln2utc(int NT, int N4, double tod)
{
	int i;
	int J = 0;
	int year = 0;
	...
	total_sec = tod;               //tb * 15 * 60

	p_utc[0] = year;
	p_utc[1] = month;
	p_utc[2] = day_in_month;
	p_utc[3] = (int)((int)total_sec / 3600);
	p_utc[4] = (int)((int)(total_sec - p_utc[3] * 3600) / 60);
	p_utc[5] = total_sec - p_utc[3] * 3600 - p_utc[4] * 60;

	utctime = timeadd(gnss_epoch2time(p_utc), -3.0 * 3600);
	if (utctime.time <= 0)
	{
		utctime.time = 0;
		utctime.sec = 0;
...
* return : 
*-----------------------------------------------------------------------------*/
void utc2gln(int* NT, int* N4, double* tod, double* p_utc)
{
	char  J;
	int i;
	int   year_cnt;
	...
	int   N4_gln;
	J = 1;
	year_cnt = (int)p_utc[0] % 4;

	if (year_cnt == 0)
	{
		day_gln = 0;

		for (i = 0; i < p_utc[1] - 1; i++)
		{
	...
	N4_gln = (int)((p_utc[0] - 1996 - J + 1) / 4 + 1);
	(*N4) = N4_gln;
	(*NT) = day_gln;
	(*tod) = (p_utc[3] * 60 + p_utc[4]) * 60 + p_utc[5] + 3.0 * 3600;
...
#else
    Fast_Rtk_matmul("NN", 3, 1, 3, 1.0, E, r, 0.0, e);
#endif
}
...
#else
    Fast_Rtk_matmul("TN", 3, 1, 3, 1.0, E, e, 0.0, r);
#endif
}
...
#else
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, E, P, 0.0, EP);
    Fast_Rtk_matmul("NT", 3, 3, 3, 1.0, EP, E, 0.0, Q);
#endif
}
...
#else
    Fast_Rtk_matmul("TN", 3, 3, 3, 1.0, E, Q, 0.0, EQ);
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, EQ, E, 0.0, P);
#endif
}
...
extern void eci2ecef(gtime_t tutc, const double *erpv, double *U, double *gmst)
{
    ...
    gnss_util_trace(4,"eci2ecef: tutc=%s\n",time_str(tutc,3));
    ...
    /* terrestrial time */
    tgps=utc2gpst(tutc_);
    t=(timediff(tgps, gnss_epoch2time(ep2000))+19.0+32.184)/86400.0/36525.0;
    t2=t*t; t3=t2*t;
...
#else
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, R1, R2, 0.0, R);
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, R, R3, 0.0, P); /* P=Rz(-z)*Ry(th)*Rz(-ze) */
#endif
...
#else
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, R1, R2, 0.0, R);
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, R, R3, 0.0, N); /* N=Rx(-eps)*Rz(-dspi)*Rx(eps) */
#endif
...
#else
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, R1, R2, 0.0, W);
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, W, R3, 0.0, R); /* W=Ry(-xp)*Rx(-yp) */
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, N, P, 0.0, NP);
    Fast_Rtk_matmul("NN", 3, 3, 3, 1.0, R, NP, 0.0, U_); /* U=W*Rz(gast)*N*P */
#endif
    
    for (i=0;i<9;i++) U[i]=U_[i];
    if (gmst) *gmst=gmst_; 
    
    gnss_util_trace(5,"gmst=%.12f gast=%.12f\n",gmst_,gast);
    gnss_util_trace(5,"P=\n"); gnss_util_tracemat(5,P,3,3,15,12);
    gnss_util_trace(5,"N=\n"); gnss_util_tracemat(5,N,3,3,15,12);
    gnss_util_trace(5,"W=\n"); gnss_util_tracemat(5,W,3,3,15,12);
    gnss_util_trace(5,"U=\n"); gnss_util_tracemat(5,U,3,3,15,12);
}
...
static void addpcv(const pcv_t *pcv, pcvs_t *pcvs)
{
    ...
    if (pcvs->nmax<=pcvs->n) {
        pcvs->nmax+=256;
        if (!(pcvs_pcv=(pcv_t *)Sys_Realloc(pcvs->pcv,sizeof(pcv_t)*pcvs->nmax))) {
            gnss_util_trace(1,"addpcv: memory allocation error\n");
			pcvs->n=pcvs->nmax=0;
            return;
        }
        pcvs->pcv=pcvs_pcv;
...
static int readngspcv(const char *file, pcvs_t *pcvs)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        gnss_util_trace(2,"ngs pcv file open error: %s\n",file);
        return 0;
    }
...
static int readantex(const char *file, pcvs_t *pcvs)
{
    ...
    gnss_util_trace(3,"readantex: file=%s\n",file);
    memset(&pcv, 0, sizeof(pcv_t));

    if (!(fp=fopen(file,"r"))) {
        gnss_util_trace(2,"antex pcv file open error: %s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
...
extern int readpcv(const char *file, pcvs_t *pcvs)
{
    ...
    gnss_util_trace(3,"readpcv: file=%s\n",file);
    
    if (!(ext=strrchr((char *)file,'.'))) ext="";
    ...
    for (i=0;i<pcvs->n;i++) {
        pcv=pcvs->pcv+i;
        gnss_util_trace(4,"sat=%2d type=%20s code=%s off=%8.4f %8.4f %8.4f  %8.4f %8.4f %8.4f\n",
                        ...
                        const pcvs_t *pcvs)
{
    ...
    gnss_util_trace(3,"searchpcv: sat=%2d type=%s\n",sat,type);
    ...
    if (sat) { /* search satellite antenna */
        ...
        for (i=0;i<pcvs->n;i++) {
            ...
            gnss_util_trace(2,"pcv without radome is used type=%s\n",type);
            return pcv;
        }
    }
    return NULL;
...
extern void readpos(const char *file, const char *rcv, double *pos)
{
    ...
    FILE *fp;
    int i,j,len,np=0;
    char buff[256], str[256] = {0};
    
    gnss_util_trace(3,"readpos: file=%s\n",file);
...
extern int readblq(const char *file, const char *sta, double *odisp)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        gnss_util_trace(2,"blq file open error: file=%s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
    ...
    fclose(fp);
    gnss_util_trace(2,"no otl parameters: sta=%s file=%s\n",sta,file);
    return 0;
}
...
extern int readerp(const char *file, erp_t *erp)
{
    ...
    gnss_util_trace(3,"readerp: file=%s\n",file);
    
    if (!(fp=fopen(file,"r"))) {
        gnss_util_trace(2,"erp file open error: file=%s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (erp->n>=erp->nmax) {
            ...
            Sys_Free(erp->data);
            erp_data=(erpd_t *)Sys_Malloc(sizeof(erpd_t)*erp->nmax);
            if (!erp_data) {
                erp->data=NULL; erp->n=erp->nmax=0;
                fclose(fp);
                return 0;
            }
            erp->data=erp_data;
...
extern int geterp(const erp_t *erp, gtime_t time, double *erpv)
{
    ...
    gnss_util_trace(4,"geterp:\n");
    
    if (erp->n<=0) return 0;
    
    mjd=51544.5+(timediff(gpst2utc(time), gnss_epoch2time(ep)))/86400.0;
...
static void uniqeph(nav_t *nav)
{
    ...
    for (i=1,j=0;i<nav->n;i++) {
        if (nav->eph[i].sat!=nav->eph[j].sat||
			...
			nav->eph[i].toes!=nav->eph[j].toes) {
			nav->eph[++j]=nav->eph[i];
        }
    }
    nav->n=j+1;
    Sys_Free(nav->eph);
    if (!(nav_eph=(eph_t *)Sys_Malloc(sizeof(eph_t)*nav->n))) {
        gnss_util_trace(1,"uniqeph malloc error n=%d\n",nav->n);
...
static void uniqgeph(nav_t *nav)
{
    ...
    gnss_util_trace(3,"uniqgeph: ng=%d\n",nav->ng);
    ...
    if (!(nav_geph=(geph_t *)Sys_Realloc(nav->geph,sizeof(geph_t)*nav->ng))) {
        gnss_util_trace(1,"uniqgeph malloc error ng=%d\n",nav->ng);
        Sys_Free(nav->geph); nav->geph=NULL; nav->ng=nav->ngmax=0;
        return;
    }
    nav->geph=nav_geph;
    nav->ngmax=nav->ng;
    
    gnss_util_trace(4,"uniqgeph: ng=%d\n",nav->ng);
#endif
}
...
static void uniqseph(nav_t *nav)
{
    ...
    gnss_util_trace(3,"uniqseph: ns=%d\n",nav->ns);
    ...
    if (!(nav_seph=(seph_t *)Sys_Realloc(nav->seph,sizeof(seph_t)*nav->ns))) {
        gnss_util_trace(1,"uniqseph malloc error ns=%d\n",nav->ns);
        Sys_Free(nav->seph); nav->seph=NULL; nav->ns=nav->nsmax=0;
        return;
    }
    nav->seph=nav_seph;
    nav->nsmax=nav->ns;
    
    gnss_util_trace(4,"uniqseph: ns=%d\n",nav->ns);
#endif
}
/* unique ephemerides ----------------------------------------------------------
...
extern int sortobs(obs_t *obs)
{
    ...
    gnss_util_trace(3,"sortobs: nobs=%d\n",obs->n);
...
extern int readnav(const char *file, nav_t *nav)
{
    ...
    gnss_util_trace(3,"loadnav: file=%s\n",file);
...
extern int savenav(const char *file, const nav_t *nav)
{
    ...
    gnss_util_trace(3,"savenav: file=%s\n",file);
...
{
    Sys_Free(obs->data); obs->data=NULL; obs->n=obs->nmax=0;
}
...
{
    if (opt&0x01) {Sys_Free(nav->eph ); nav->eph =NULL; nav->n =nav->nmax =0;}
#if 0
    if (opt&0x02) {Sys_Free(nav->geph); nav->geph=NULL; nav->ng=nav->ngmax=0;}
    if (opt&0x04) {Sys_Free(nav->seph); nav->seph=NULL; nav->ns=nav->nsmax=0;}
    if (opt&0x08) {Sys_Free(nav->peph); nav->peph=NULL; nav->ne=nav->nemax=0;}
    if (opt&0x10) {Sys_Free(nav->pclk); nav->pclk=NULL; nav->nc=nav->ncmax=0;}
    if (opt&0x20) {Sys_Free(nav->alm ); nav->alm =NULL; nav->na=nav->namax=0;}
    if (opt&0x40) {Sys_Free(nav->tec ); nav->tec =NULL; nav->nt=nav->ntmax=0;}
    if (opt&0x80) {Sys_Free(nav->fcb ); nav->fcb =NULL; nav->nf=nav->nfmax=0;}
#endif
}
...
static unsigned int tick_trace=0; /* tick time at gnss_util_traceopen (ms) */
static gtime_t time_trace={0};  /* time at gnss_util_traceopen */
static lock_t lock_trace;       /* lock for trace */

static unsigned int tickget(void)
{
  struct timeval tv = { 0 };
...
extern void gnss_util_traceopen(const char *file)
{
    gtime_t time=utc2gpst(timeget());
    char path[1024];
...
extern void gnss_util_traceclose(void)
{
    if (fp_trace&&fp_trace!=stderr) fclose(fp_trace);
    fp_trace=NULL;
    file_trace[0]='\0';
}
extern void gnss_util_tracelevel(int level)
{
    level_trace=level;
}
extern void gnss_util_trace(int level, const char *format, ...)
{
    va_list ap;
...
extern void gnss_util_tracet(int level, const char *format, ...)
{
    va_list ap;
...
extern void gnss_util_tracemat(int level, const double *A, int n, int m, int p, int q)
{
    if (!fp_trace||level>level_trace) return;
    matfprint(A,n,m,p,q,fp_trace); fflush(fp_trace);
}
extern void gnss_util_traceobs(int level, const obsd_t *obs, int n)
{
    char str[64],id[16];
    int i;
...
extern void gnss_util_tracenav(int level, const nav_t *nav)
{
#if 0
...
#if 0
extern void gnss_util_tracegnav(int level, const nav_t *nav)
{
    char s1[64],s2[64],id[16];
    int i;
...
#endif
extern void gnss_util_tracehnav(int level, const nav_t *nav)
{
#if 0
    char s1[64],s2[64],id[16];
    int i;
...
extern void gnss_util_tracepeph(int level, const nav_t *nav)
{
#if 0
    char s[64],id[16];
    int i,j;
...
extern void gnss_util_tracepclk(int level, const nav_t *nav)
{
#if 0
    char s[64],id[16];
    int i,j;
...
extern void gnss_util_traceb(int level, const unsigned char *p, int n)
{
    int i;
    if (!fp_trace||level>level_trace) return;
...
extern int execcmd(const char *cmd)
{
    ...
    gnss_util_trace(3,"execcmd: cmd=%s\n",cmd);
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    gnss_util_trace(3,"expath  : path=%s nmax=%d\n",path,nmax);
    ...
    for (i=0;i<n;i++) gnss_util_trace(3,"expath  : file=%s\n",paths[i]);
    ...
    while ((d=readdir(dp))) {
        ...
        snprintf(s1,sizeof(s1),"^%s$",d->d_name);
        snprintf(s2,sizeof(s2),"^%s$",file);
...
extern void createdir(const char *path)
{
    ...
    gnss_util_tracet(3,"createdir: path=%s\n",path);
...
static int repstr(char *str, const char *pat, const char *rep)
{
    ...
    strcpy(r,p);
    strcpy(str,buff);*/
    /*the size of buff may be greater that the str,and the function not be used in the loaction engine 2.0*/
    return 1;
}
                   ...
                   const char *base)
{
    ...
    if (time.time!=0) {
        ...
        doy=(int)floor(timediff(time, gnss_epoch2time(ep0))/86400.0)+1;
                    ...
                    gtime_t te, const char *rov, const char *base)
{
    ...
    gnss_util_trace(3,"reppaths: path =%s nmax=%d rov=%s base=%s\n",path,nmax,rov,base);
    ...
    time= gnss_gpst2time(week,floor(tow/tint)*tint);
    ...
    for (i=0;i<n;i++) gnss_util_trace(3,"reppaths: rpath=%s\n",rpath[i]);
    return n;
}
    ...
    if (sys==SYS_GLO) {
        ...
        if (0<=frq&&frq<=1) {
            for (i=0;i<nav->ng;i++) {
                ...
                if (nav->geph[i].frq>6 || nav->geph[i].frq<-7) continue;
...
{
	if (sys==SYS_GLO) {
		if (fcn>6 || fcn<-7) return 0.0;
		if (frq==0) {
			return CLIGHT/(FREQ1_GLO+DFRQ1_GLO*fcn);
		}
		else if (frq==1) {
			return CLIGHT/(FREQ2_GLO+DFRQ2_GLO*fcn);
		}
		else if (frq==2) { /* L3 */
			return CLIGHT/FREQ3_GLO;
		}
...
extern double geodist(const double *rs, const double *rr, double *e)
{
    ...
    if (asg_norm(rs,3)<RE_WGS84) return -1.0;
...
#else
    Fast_Rtk_matmul("NT", 4, 4, n, 1.0, H, H, 0.0, Q);
#endif
    if (!matinv(Q,4)) {
                       ...
                       const double *azel)
{
    ...
    if (asg_norm(ion,8)<=0.0) ion=ion_default;
...
#endif
    gnss_util_trace(5,"tropmapf: pos=%10.6f %11.6f %6.1f azel=%5.1f %4.1f\n",
...
#ifdef IERS_MODEL
    mjd=51544.5+(timediff(time, gnss_epoch2time(ep)))/86400.0;
    lat=pos[0];
    lon=pos[1];
                     ...
                     int opt, double *dant)
{
    ...
    gnss_util_trace(5,"antmodel: azel=%6.1f %4.1f opt=%d\n",azel[0]*R2D,azel[1]*R2D,opt);
    ...
    gnss_util_trace(5,"antmodel: dant=%6.3f %6.3f\n",dant[0],dant[1]);
}
...
extern void antmodel_s(const pcv_t *pcv, double nadir, double *dant)
{
    ...
    gnss_util_trace(4,"antmodel_s: nadir=%6.1f\n",nadir*R2D);
    
    for (i=0;i<NFREQ;i++) {
        dant[i]=interpvar(nadir*R2D*5.0,pcv->var[i]);
    }
    gnss_util_trace(5,"antmodel_s: dant=%6.3f %6.3f\n",dant[0],dant[1]);
}
...
static void sunmoonpos_eci(gtime_t tut, double *rsun, double *rmoon)
{
    ...
    gnss_util_trace(4,"sunmoonpos_eci: tut=%s\n",time_str(tut,3));
    
    t=timediff(tut, gnss_epoch2time(ep2000))/86400.0/36525.0;
    ...
    if (rsun) {
        ...
        gnss_util_trace(5,"rsun =%.3f %.3f %.3f\n",rsun[0],rsun[1],rsun[2]);
    }
    ...
    if (rmoon) {
        ...
        gnss_util_trace(5,"rmoon=%.3f %.3f %.3f\n",rmoon[0],rmoon[1],rmoon[2]);
    }
}
                       ...
                       double *rmoon, double *gmst)
{
    ...
    gnss_util_trace(4,"sunmoonpos: tutc=%s\n",time_str(tutc,3));
...
#else
    if (rsun) Fast_Rtk_matmul("NN", 3, 1, 3, 1.0, U, rs, 0.0, rsun);
    if (rmoon) Fast_Rtk_matmul("NN", 3, 1, 3, 1.0, U, rm, 0.0, rmoon);
#endif
    if (gmst ) *gmst=gmst_;
}
/* carrier smoothing -----------------------------------------------------------
...
extern void csmooth(obs_t *obs, int ns)
{
    ...
    gnss_util_trace(3,"csmooth: nobs=%d,ns=%d\n",obs->n,ns);
    ...
    for (i=0;i<obs->n;i++) {
        ...
        for (j=0;j<NFREQ;j++) {
            ...
            if (n[r-1][s-1][j]==0) Ps[r-1][s-1][j]=p->P[j];
                ...
                Ps[r-1][s-1][j]=p->P[j]/ns+(Ps[r-1][s-1][j]+dcp)*(ns-1.0)/ns;
            }
...
extern int rtk_uncompress(const char *file, char *uncfile)
{
    ...
    gnss_util_trace(3,"rtk_uncompress: file=%s\n",file);
    ...
    gnss_util_trace(3,"rtk_uncompress: stat=%d\n",stat);
    return stat;
}
#endif
...
static void buff2sysopts(prcopt_t* popt)
{
  double pos[3], * rr;
  int i, * ps;

  if (popt->elmin > PI / 2)        popt->elmin = popt->elmin * D2R;
  if (popt->elmaskar > PI / 2)     popt->elmaskar = popt->elmaskar * D2R;
  if (popt->elmaskhold > PI / 2)   popt->elmaskhold = popt->elmaskhold * D2R;

  for (i = 0; i < 2; i++) {
    ps = i == 0 ? &popt->rovpos : &popt->refpos;
    rr = i == 0 ? popt->ru : popt->rb;
...
extern void resetsysopts_lite(prcopt_t* popt)
{
  memset(popt, 0, sizeof(*popt));
  gnss_util_trace(3, "resetsysopts:\n");

  popt->mode = PMODE_KINEMA;
  popt->soltype = 0;
  popt->nf = 1;
  popt->navsys = SYS_GPS | SYS_CMP | SYS_GAL | SYS_QZS;
  popt->elmin = 10.0 * D2R;
  popt->sateph = 0;
  popt->modear = 3;
  popt->glomodear = 0;
...
*-----------------------------------------------------------------------------*/
extern void getsysopts_lite(prcopt_t* popt)
{
  buff2sysopts(popt);
}

/* time to gps time ------------------------------------------------------------
* convert asg_gtime_t struct to week and tow in gps time
* args   : asg_gtime_t t        I   asg_gtime_t struct
*          int    *week     IO  week number in gps time (NULL: no output)
* return : time of week in gps time (s)
*-----------------------------------------------------------------------------*/
extern double rtklib_time2gpst(asg_gtime_t t, int* week)
{
  asg_gtime_t t0 = asg_epoch2time(gpst0);
  time_t sec = t.time - t0.time;
  int w = (int)(sec / (86400 * 7));
...
/* gpstime to utc --------------------------------------------------------------
* convert gpstime to utc considering leap seconds
* args   : asg_gtime_t t        I   time expressed in gpstime
* return : time expressed in utc
* notes  : ignore slight time offset under 100 ns
*-----------------------------------------------------------------------------*/
asg_gtime_t asg_gpst2utc(asg_gtime_t t)
{
  ...
  for (i = 0; leaps[i][0] > 0; i++) {
    tu = asg_timeadd(t, leaps[i][6]);
    if (asg_timediff(tu, asg_epoch2time(leaps[i])) >= 0.0) return tu;
  }
  return t;
}
/* utc to gpstime --------------------------------------------------------------
* convert utc to gpstime considering leap seconds
* args   : asg_gtime_t t        I   time expressed in utc
* return : time expressed in gpstime
* notes  : ignore slight time offset under 100 ns
*-----------------------------------------------------------------------------*/
asg_gtime_t asg_utc2gpst(asg_gtime_t t)
{
  ...
  for (i = 0; leaps[i][0] > 0; i++) {
    if (asg_timediff(t, asg_epoch2time(leaps[i])) >= 0.0) return asg_timeadd(t, -leaps[i][6]);
  }
  return t;
}

/* convert calendar day/time to time -------------------------------------------
* convert calendar day/time to asg_gtime_t struct
* args   : double *ep       I   day/time {year,month,day,hour,min,sec}
* return : asg_gtime_t struct
* notes  : proper in 1970-2037 or 1970-2099 (64bit time_t)
*-----------------------------------------------------------------------------*/
extern asg_gtime_t asg_epoch2time(const double* ep)
{
  const int doy[] = { 1,32,60,91,121,152,182,213,244,274,305,335 };
  asg_gtime_t time = { 0 };
  int days, sec, year = (int)ep[0], mon = (int)ep[1], day = (int)ep[2];
...
/* time to calendar day/time ---------------------------------------------------
* convert asg_gtime_t struct to calendar day/time
* args   : asg_gtime_t t        I   asg_gtime_t struct
*          double *ep       O   day/time {year,month,day,hour,min,sec}
* return : none
* notes  : proper in 1970-2037 or 1970-2099 (64bit time_t)
*-----------------------------------------------------------------------------*/
extern void asg_time2epoch(asg_gtime_t t, double* ep)
{
  const int mday[] = { /* # of days in a month */
      31,28,31,30,31,30,31,31,30,31,30,31,31,28,31,30,31,30,31,31,30,31,30,31,
      31,29,31,30,31,30,31,31,30,31,30,31,31,28,31,30,31,30,31,31,30,31,30,31
...
/* add time --------------------------------------------------------------------
* add time to asg_gtime_t struct
* args   : asg_gtime_t t        I   asg_gtime_t struct
*          double sec       I   time to add (s)
* return : asg_gtime_t struct (t+sec)
*-----------------------------------------------------------------------------*/
extern asg_gtime_t asg_timeadd(asg_gtime_t t, double sec)
{
  double tt;
...
/* time difference -------------------------------------------------------------
* difference between asg_gtime_t structs
* args   : asg_gtime_t t1,t2    I   asg_gtime_t structs
* return : time difference (t1-t2) (s)
*-----------------------------------------------------------------------------*/
extern double asg_timediff(asg_gtime_t t1, asg_gtime_t t2)
{
  return difftime(t1.time, t2.time) + t1.sec - t2.sec;
}
#endif